...
public class MyDatabaseHelper extends SQLiteOpenHelper {
    private final static String CREATE_NOTES = "create table notes (_id integer primary key autoincrement, title text, content text, time text, id integer, bmob_id text)";
     ...
     */
    public void insert(String title, String content, String time, long id, String bmob_id) {
        //插入一条数据
     ...
     * @param bmob_id bmob_id
     */
    public void update(String title, String content, String time, long id, String bmob_id) {
        //修改一条数据
        ContentValues values = new ContentValues();
        values.put("title", title);
        values.put("content", content);
    ...
    public LinkedList<Note> query() {
        ...
        if (cursor.moveToNext()) {
            do {
                ...
                Note note = new Note(title, content, time, id,bmob_id);
                data.add(note);
                Log.d("MainActivity", id + ": title:" + title + ", content:" + content + ", time:" + time+", bmob_id:"+bmob_id);
    ...
    public LinkedList<Note> query(String bmob_id) {
        //查询全部数据，返回一个cursor，从cursor取出数据
        Cursor cursor = db.rawQuery("select * from notes where bmob_id like ?", new String[]{MyAdapter.EMPTY_BMOB_ID});
        LinkedList<Note> data = new LinkedList<>();
        if (cursor.moveToNext()) {
            do {
                String title = cursor.getString(cursor.getColumnIndex("title"));
    ...
    public Note query(long id) {
        ...
        if (cursor.moveToNext()) {
            do {
                ...
                String time = cursor.getString(cursor.getColumnIndex("time"));
                String bmob_id = cursor.getString(cursor.getColumnIndex("bmob_id"));
                note = new Note(title, content, time, id, bmob_id);
                Log.d("query", id + ": " + title + " " + content + " " + time + " " + bmob_id);
            } while (cursor.moveToNext());
